...
def bfs(n, info):    
    ...
    while q:
        focus, arrow = q.popleft()  #지금 몇 점에 화살 쏠건지, 현재 화살 상황
        ...
        elif focus == 10:  # 종료조건 3) 화살 덜 쏜 경우 -> 남은 화살을 전부 마지막 과녁에 쏴준다.
            tmp = arrow.copy()
            tmp[focus] = n - sum(tmp)
            q.append((-1, tmp))
...
def solution(n, info):
    ...
    if not winList: #라이언 이기는 경우 없음
        return [-1]
    elif len(winList) == 1:
        return winList[0]
    else:   #낮은 점수 더 많이 맞힌 경우(높은 점수부터 시작했으므로 마지막 원소) 리턴
        return winList[-1]